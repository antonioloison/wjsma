"""
The SalienceMapMethod attack and returning the adversarial sample and
...
class LogMapMethod(Attack):
    """
    Improvement of the Jacobian-based Saliency Map Method (Papernot et al. 2016).
    ...
    def __init__(self, model, sess=None, dtypestr='float32', **kwargs):
        """
        Create a LogMapMethod instance.
        Note: the model parameter should be an instance of the
        cleverhans.model.Model abstraction provided by CleverHans.
        """

        super(LogMapMethod, self).__init__(model, sess, dtypestr, **kwargs)
    ...
    def generate(self, x, **kwargs):
        ...
        if self.symbolic_impl:
            ...
            raise NotImplementedError("The jsma_batch function has been removed."
                                      " The symbolic_impl argument to LogMapMethod will be removed"
                                      ...
                                      " LogMapMethod.generate_np() instead."
                                      )
...
def jsma_symbolic(x, y_target, model, theta, gamma, clip_min, clip_max):
    ...
    def body(x_in, y_in, domain_in, i_in, cond_in, predictions):
        ...
        preds = tf.reshape(preds, shape=[nb_classes, -1, 1])
        preds_target = reduce_sum(preds * target_class, axis=0)
        ones = tf.ones([1])
        first_prod = (ones - preds_target) * grads_target
        ...
        cond_float = tf.cast(cond, tf_dtype)
